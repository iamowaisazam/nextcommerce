          ...
          <div className="container">
            <div className="row">
              <div className="col-12">
                <h3 className="bread-crumb-title">{category.title}</h3>
                  ...
                  return <div className="col-xl-2 col-lg-3 col-sm-6 col-md-4" key={key} >
                    <div className="product-card">
                        <Link className="thumb"  href={`/products/${item.slug}`} >
                          <a href= ""><img src={`${category.imgpath}${item.thumbnail}`}  /> </a>
                        </Link>
...
export async function getServerSideProps({query}) {
    ...
    const {slug} = query;
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/items/category/${slug}`);  
